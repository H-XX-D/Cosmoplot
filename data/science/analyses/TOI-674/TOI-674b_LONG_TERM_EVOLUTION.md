# TOI-674b Long-Term Evolution Projection

## 1. Current State Summary

### Planetary Parameters and Classification

TOI-674b is a Neptune-like with distinctive characteristics that shape its evolutionary trajectory. With a mean density of 0.860 g/cm³, it occupies a unique position in the density-temperature phase space of exoplanet atmospheres, directly influencing how stellar radiation impacts its long-term atmospheric stability.

**Fundamental Properties:**
- **Mass**: 0.074 M_J (equivalently 23.6 M_E)
- **Radius**: 0.468 R_J (5.25 R_E)
- **Mean Density**: ρ = 0.860 g/cm³
- **Equilibrium Temperature**: T_eq = 635 K
- **Orbital Period**: 1.98 days
- **Semi-major Axis**: a = 0.0250 AU
- **Host Star**: M2V dwarf (T_eff = 3514 K, L★ = 0.0243 L☉)
- **Stellar XUV Flux**: F_XUV ≈ 65 erg/s/cm²
- **Atmospheric Scale Height**: H ≈ 3.1 km

### Current Atmospheric Characteristics

TOI-674b's current atmosphere represents the primordial or evolved state depending on its age and history. The planet maintains a hydrogen-dominated envelope with trace heavier species that provide valuable spectroscopic signatures.
...
Surface gravity at 0.468 R_J: g = 8.8 m/s²

Escape velocity: v_esc = √(2GM/R) = **23.99 km/s**

Thermal velocity at 635 K: v_th = √(3k_B T/m_H) = **3.968 km/s**

**Escape Parameter**: λ = v_esc / v_th = **6.0**

The physically meaningful thermal-escape diagnostic is the Jeans parameter lambda_J = (3/2)(v_esc/v_th)^2 = **55**. Jeans escape is negligible for lambda_J >~ 15 (thermally bound) and only becomes hydrodynamic blow-off for lambda_J <~ 2-3. Close-in giants therefore lose mass mainly through XUV-driven energy-limited escape, not thermal Jeans escape.

**Escape energy per H atom**: E_b = 1/2 m_H v_esc^2 = **3004 meV** (3.00 eV)

For comparison the mean thermal energy is E_th = 3k_B T = 164 meV, far below the escape energy, confirming the atmosphere is thermally bound.

TOI-674b's Jeans escape parameter is lambda_J = (3/2)(v_esc/v_th)^2 = 55 (>> 15), so thermal (Jeans) escape is negligible and the planet retains its hydrogen envelope; any loss proceeds through XUV-driven energy-limited escape and is modest over Gyr timescales.
...
Over the first billion years, TOI-674b experiences atmospheric erosion primarily through XUV photoevaporation. The dominant mechanism depends on the planet's balance between stellar heating and gravitational binding.
...
1. **Ionization**: Stellar EUV photons (10-100 nm) ionize neutral hydrogen atoms
   ...
   - XUV flux received: F_XUV = 65 erg/s/cm²
...
Substituting values:
$$\dot{M} \approx 0.07 \times \frac{65 \times 6.24 \times 10^{11}}{3.0} \times 4\pi \times (0.468 R_J)^2$$
...
- Remaining atmosphere: 0.0004 M_J (99.9% retained)
...
**Density Evolution:**
- Current density: 0.860 g/cm³
- Density at 5 Gyr: 0.860 g/cm³ (0.0% increase)
...
- Escape parameter now: λ ≈ 5.9 (slight decline from 6.0)
...
At 10 billion years, TOI-674b's atmosphere reaches an approximately equilibrial state. The escape rate becomes negligible and the atmosphere stabilizes.
...
- Final mass: 0.074 M_J (essentially unchanged)
- Final density: 0.860 g/cm³
...
TOI-674b will **continue active atmospheric loss** beyond 10 Gyr.

### Evolutionary Category and Final Fate

TOI-674b falls into the **Actively Eroding** evolutionary category.
...
Ionization leaves behind energetic photoelectrons that dissipate energy through collisions, heating the local atmosphere from ≈600 K to ≈1135 K. This heating causes thermal expansion.

**Stage 3: Hydrodynamic Expansion**
The heated atmosphere expands, creating a density gradient that drives outflow. The escape velocity 23.99 km/s competes with the thermal velocity 3.968 km/s. Since v_esc >> v_th (escape parameter 6.0), gravity weakly constrains the flow.
...
The binding energy E_b = 1/2 m_H v_esc^2 ≈ 3004 meV represents the gravitational potential well depth per hydrogen atom. This determines how much thermal energy is needed to escape:

**Energy Balance:**
- Thermal energy available: E_th = 3k_B T ≈ 164 meV
- Escape energy per H: E_b = 1/2 m_H v_esc^2 ≈ 3004 meV
- Ratio: E_th/E_b = 0.055 (<<1 strongly bound)
...
TOI-674b's density of 0.860 g/cm³ places it in the moderate-density category.
...
### Why TOI-674b Evolves as Predicted

**Key Evolutionary Drivers:**

1. **Escape Parameter**: λ = 6.0
   - Controls atmospheric retention: Poor retention expected

2. **XUV Flux**: 65 erg/s/cm²
...
4. **Comparative Evolution**: TOI-674b's evolution rate is among the slowest in its temperature class

---

## Conclusion

TOI-674b represents a template for active atmospheric erosion among exoplanet atmospheres. Its 10 billion year trajectory demonstrates how the interplay between stellar heating (65 erg/s/cm²), planetary gravity (λ = 6.0), and initial mass (0.0004 M_J) determines long-term atmospheric fate.

**Key Takeaways:**

1. **Escape Parameter Dominance**: The escape parameter λ = 6.0 is the single best predictor of TOI-674b's evolutionary path

2. **Asymptotic Stability**: After 5-10 Gyr, the escape rate becomes negligible, and the atmosphere stabilizes

3. **Density-Gravity Coupling**: TOI-674b's density of 0.860 g/cm³ creates strong gravitational retention

4. **Observable Evolution**: Subtle radius and composition changes expected over decade-timescales with precision spectroscopy

For exoplanet scientists, TOI-674b provides a clear example of how vulnerable planets can be to atmospheric erosion. JWST observations over coming years will test these predictions and refine our understanding of long-term exoplanet evolution.
